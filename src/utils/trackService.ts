...
const isFavorite = (track: DeezerTrack): boolean => {
  const favorites = loadFavorites();
  const filtered = favorites.filter(favoriteTrack => favoriteTrack.id === track.id);
  return filtered.length > 0;
};

const removeFavorite = (track: DeezerTrack): void => {
  const favorites = loadFavorites();
  const updatedFavorites = favorites.filter(favoriteTrack => favoriteTrack.id !== track.id);
